...
public class ProcessLogInterceptor implements HandlerInterceptor {
	...
	private PHCSMP_Process_Log jugeMethod(String method, String sdate,
			String ddate, HttpServletRequest arg0,
			HttpServletResponse response, String suspectId) throws IOException {
		...
		case "readRFID": {
			...
			case 5: {
				...
				if (temporaryLeaveService.IsTemporaryLeaveReturn(suspectId) == null
						&& completeIsNull(suspectId) != null) {
					log = loginfoService.searchEmpcomplete(suspectId);
		...
		case "addSuspectInfor": {
			...
			if (arg0.getAttribute("suspected_Cause") != null) {
				suspected_Cause = ((String) arg0
						.getAttribute("suspected_Cause"));
			}
		...
		case "addCheckPersonInfor": {
			...
			if (staffIsNull(suspectId) != null) {
				log = loginfoService.searchEmpstaff(suspectId);
				log.setStaff_ID(Integer.parseInt((String) arg0
						.getAttribute("staff_Name")));
			}
			// 完整性检查
			...
			System.out.println(log.getComplete()
					+ "--------------------complete");
			break;
		}
		// 信息采集提交
		...
		case "addActivityRecordInfor": {
			...
			if (arg0.getAttribute("activity_Record") != null) {
				log.setSuspect_active((String) arg0
		...
		case "addTemporaryLeaveInfor": {
			...
			// System.out.println(log+"-----------------------------------------------log");
			if (staffIsNull(suspectId) != null
					&& arg0.getAttribute("staff_ID") != null) {
				// log = loginfoService.searchEmpstaff(suspectId);
				log.setStaff_ID((int) arg0.getAttribute("staff_ID"));
	...
	private void checkWay(HttpServletRequest request,
		...
		if (videoStetus == "1") {
			response.getWriter().write("<script>alert('回路正常，开始录像');</script>");
			response.getWriter().flush();

		} else if (videoStetus == "0") {
			response.getWriter().write("<script>alert('回路饱和，录像失败');</script>");
			response.getWriter().flush();
		}
	}
	...
	private PHCSMP_Process_Log endtimeIsZero(String suspectId) {
		try {
			PHCSMP_Process_Log processLog = loginfoService
					.searchEmpEndTime(suspectId);
