...
//		}
	...
	public boolean preHandle(HttpServletRequest arg0, HttpServletResponse arg1,
		...
		String suspectId = arg0.getParameter("suspectID");
		if(arg == "addLeaveRecordInfor"||arg.equals("addLeaveRecordInfor")){
			log = loginfoService.searchEmpcomplete(suspectId);
			
			if (staffIsNull(suspectId) != null) {
//				log = loginfoService.searchEmpstaff(suspectId);
				log.setStaff_ID(Integer.parseInt(arg0.getParameter("tempLeave_manager")));
			}
//			System.out.println(log+"---------------------------------log");
			...
			if (log.getStaff_ID() != 0) {
				updateporStaff(log);
				updateNew(log);
			}
			if (log.getEnd_Time() != "0-0") {
				updateprocess(log);
			}
		}
